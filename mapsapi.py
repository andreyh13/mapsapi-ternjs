...
def processParameterOptions(options):
    ...
    if options.startswith("Array<"):
        t = processParameterOptions(
            options[len("Array<"): options.rindex(">")])
        for y in t:
            res.append("[" + y + "]")
    elif options.startswith("MVCArray<"):
        t = processParameterOptions(
            options[len("MVCArray<"): options.rindex(">")])
        for y in t:
            res.append("[" + y + "]")
    else:
        a = options.split("|", 1)
...
def processCallbackFunction(pfunc):
    ...
    for o in l_opt:
        ...
        res += (', ' if res != '' else '') + 'p' + str(count) + ': ' + \
            ('?' if len(oo) > 1 else checkParameterType(oo[0].strip()))
        count += 1
    res = 'fn(' + res + ')'
    return res
...
def processFuncParameters(sparams):
    ...
    for p in arr_p:
        ...
        res += (', ' if res != '' else '') + k.strip() + ': ' + \
            ('?' if len(d_params[k]) > 1 else checkParameterType(
                d_params[k][0].strip()))
    # print d_params
    return res
            ...
            for tr in child.childNodes:
                if tr.tagName == 'tr':
                    ...
                    if(ftype != 'None' and ftype != ''):
                        resf += ' -> ' + checkParameterType(
                            l_classes[ftype] if ftype in l_classes.keys() else processReturnType(ftype))
        ...
        if child.tagName == 'tbody':
            for tr in child.childNodes:
                if tr.tagName == 'tr':
                    ...
                    p_type = checkParameterType(
                        l_classes[ptype] if ptype in l_classes.keys() else processReturnType(ptype))
                    ...
                    for n2 in n1.childNodes:
                        if n2.tagName == 'article' and n2.getAttribute('class') == 'devsite-article':
                            for n3 in n2.childNodes:
                                if n3.tagName == 'div' and n3.hasAttribute('class') and 'devsite-article-body' in n3.getAttribute('class'):
                                    wrapper = n3
                                    break
    ...
    if wrapper is not None:
        for node in wrapper.childNodes:
            if node.tagName == 'div' and node.hasAttribute('itemscope') and node.hasAttribute('itemtype') and node.getAttribute('itemtype') == 'http://developers.google.com/ReferenceObject':
                processOneRefObject(
                    node, data_struc, l_classes, l_objects, subpath)
...
str_json = json.dumps(data_struc, sort_keys=True,
                      indent=4, separators=(',', ': '))
